...
Http.prototype.sendGetRequest = function (queryBuilder) {
  ...
  if (!(queryBuilder instanceof QueryBuilder)) {
    throw new KogoAdminException('Invalid query builder passed. Instance of QueryBuilder expected');
  }
...
Http.prototype.sendPostRequest = function (entity) {
  ...
  if (!(entity instanceof Model)) {
    throw new KogoAdminException('Invalid model passed. Instance of Model expected');
  }
  ...
  return HttpClient.request(requestObject)
    .then(function (response) {
      ...
      if (response && response.httpStatusCode && response.httpStatusCode > 399) {
