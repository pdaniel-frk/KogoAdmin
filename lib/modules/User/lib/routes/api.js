...
var fs                  = require('fs');
var UserModuleException = require('../exceptions');
...
exports.create = function (req, res) {
  ...
  if (!user.avatar) {
    var userModuleException = new UserModuleException();
    userModuleException.appendError(
      'avatar',
      userModuleException.ERROR_AVATAR_REQUIRED,
      'Avatar is required'
    );
    return res.status(400).json(userModuleException.export());
  }
  ...
  usersService.save(user)
    .then(function (user) {
      ...
      if (error && error.data && error.data.code == 'ER_DUP_ENTRY') {
        var userModuleException = new UserModuleException();
        userModuleException.appendError(
          'username',
          userModuleException.ERROR_USERNAME_ALREADY_IN_USE,
          'Provided username already exists'
        );
        return res.status(409).json(userModuleException.export());
      }
